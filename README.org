* Sudoku Solver
This is a Sudoku solver written in Zig.

** Building
The solver can be build with a simple zig command. The required Zig version is ~0.15.1~.
#+begin_src sh
...
If a cell is empty, we indicate this by writing a dot =[.]= symbol.
